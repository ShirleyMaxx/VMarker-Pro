...
class HRNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, multi_scale_output=cfg.diffusion.agg_feat)
    ...
    def forward(self, x, return_feat=False):
        ...
        for i in range(self.stage4_cfg.num_branches):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        y_list = self.stage4(x_list)    # (B, 48, H, W)
    ...
    def init_weights(self):
        ...
        logging.info('=> init HRNet weights from normal distribution')
...
def _make_cls_head():
    ...
    pre_stage_channels = cfg.model.hrnet.stage4.num_channels

    # Increasing the #channels on each resolution
    # from C, 2C, 4C, 8C to 128, 256, 512, 102
    incre_modules = []
    for i, channels in enumerate(pre_stage_channels):
        incre_module = _make_cls_layer(head_block,
                                            channels,
                                            head_channels[i],
                                            1,
                                            stride=1)
    ...
    final_layer = nn.Sequential(
        nn.Conv2d(
            ...
            out_channels=cfg.model_diff.condplane,
            kernel_size=1,
            stride=1,
            padding=0
        ),
        nn.BatchNorm2d(cfg.model_diff.condplane, momentum=BN_MOMENTUM),
        nn.ReLU(inplace=True)
    )

    return incre_modules, downsamp_modules, final_layer

def _make_cls_layer(block, inplanes, planes, blocks, stride=1):
    downsample = None
